...
{
  curl -s http://localhost | grep "Welcome to nginx!" > /dev/null
...
{
  local output=$(curl -s http://localhost/notes.txt)
...
{
  curl -s http://localhost:81 | grep "Welcome to nginx!" > /dev/null
...
{
  local output=$(curl -s http://localhost:81/notes.txt)
