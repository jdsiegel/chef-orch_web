...
{
  curl -s https://localhost:444 --cacert /etc/nginx/ssl/ssl_from_file.cert > /dev/null
...
{
  curl -s https://localhost:444 --cacert /etc/nginx/ssl/ssl_from_file.cert | grep "Welcome to nginx!" > /dev/null
