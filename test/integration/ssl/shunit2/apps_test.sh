...
{
  curl -s https://localhost --cacert /etc/nginx/ssl/sslapp.crt > /dev/null
...
{
  curl -s https://localhost --cacert /etc/nginx/ssl/sslapp.crt | grep "Welcome to nginx!" > /dev/null
...
{
  local output=$(curl -s https://localhost/notes.txt --cacert /etc/nginx/ssl/sslapp.crt)
...
{
  curl -s https://localhost:444 --cacert /etc/ssl/ssl_from_file.crt > /dev/null
...
{
  curl -s https://localhost:444 --cacert /etc/ssl/ssl_from_file.crt | grep "Welcome to nginx!" > /dev/null
